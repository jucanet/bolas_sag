...
function procesa()
{
	...
	*/
	...
	$RADIO_INI=5*$PULGADA;
	//Obtenemos la densidad para: Carbon Steel SAE 1045
	$RO=7.85;	//en Kg/m3
	//Obtenemos Pi
	$PI=pi();
	...
	if ( 
			($resultado[0][2]>512 || $resultado[0][2]<-511) || 
			($resultado[0][3]>512 || $resultado[0][3]<-511) || 
			($resultado[0][4]>512 || $resultado[0][4]<-511)
		)
		$ERROR=true;
	//Velocidad
	if (
	...
	$acelx=($resultado[0][2]*$g*2)/512;
	$acely=($resultado[0][3]*$g*2)/512;
	$acelz=($resultado[0][4]*$g*2)/512;
	//Almacenamos las velocidades
